...
lv_obj_t *ui_SA2DP;
lv_obj_t *ui_sa2_title;

void ui_event_sa2_edit(lv_event_t *e);

lv_obj_t *ui_sa2_back;
lv_obj_t *light_Arc1;
lv_obj_t *ui_SA2DP_Label5;
lv_obj_t *light_Arc1_label;
...
static lv_obj_t *create_vol(lv_obj_t *parent);
...
lv_obj_t *_lv_demo_music_main_create(lv_obj_t *parent)
{
    ...
    //    lv_obj_t * vol_box = create_vol(cont);
    ...
    //    lv_obj_set_grid_cell(vol_box, LV_GRID_ALIGN_STRETCH, 0, 1, LV_ALIGN_CENTER, 4, 1);
    ...
    //    lv_obj_set_grid_cell(vol_box, LV_GRID_ALIGN_STRETCH, 0, 1, LV_GRID_ALIGN_CENTER, 5, 1);
...
static void event_handler_win1(lv_event_t *e)
{
...
static void event_handler_win2(lv_event_t *e)
{
    lv_obj_t *obj = lv_event_get_target(e);
    LV_UNUSED(obj);
    LV_LOG_USER("Button %d clicked", (int)lv_obj_get_index(obj));
...
void lv_example_win_1(void)
{
    win1 = lv_win_create(lv_scr_act(), 40);
    lv_obj_t *btn;
    lv_win_add_title(win1, "Setup Volume");
    btn = lv_win_add_btn(win1, LV_SYMBOL_CLOSE, 60);
    lv_obj_add_event_cb(btn, event_handler_win1, LV_EVENT_CLICKED, NULL);
    lv_obj_t *cont = lv_win_get_content(win1); /*Content can be added here*/
    lv_obj_t *vol = lv_slider_create(cont);
    lv_obj_set_height(vol, 15);
    lv_obj_set_width(vol, 200);
    lv_obj_align(vol, LV_ALIGN_CENTER, 0, -5);

    lv_obj_add_event_cb(vol, slider_event_cb, LV_EVENT_VALUE_CHANGED, NULL);
    slider_label = lv_label_create(vol);
    lv_label_set_text(slider_label, "20");
    lv_slider_set_range(vol, 0, 20);
    lv_bar_set_value(vol, 20, LV_ANIM_OFF);
}
...
static void value_changed_event_cb(lv_event_t *e)
{
    ...
    lv_label_set_text_fmt(light_Arc1_label, "%d%", lv_arc_get_value(arc));

    val = lv_arc_get_value(arc);
    char vals[10];
    itoa(val, vals, 10);
    Serial1.println(vals);
    lv_label_set_text(light_Arc1_label, vals);
    int pwmp = val * 2.55f;
    Serial.println(pwmp);
    ledcSetup(0, 600, 8);
    ledcAttachPin(27, 0);
...
void lv_example_win_2(void)
{
    win2 = lv_win_create(lv_scr_act(), 40);
    lv_obj_t *btn;
    lv_win_add_title(win2, "Setup Luminance");
    btn = lv_win_add_btn(win2, LV_SYMBOL_CLOSE, 60);
    lv_obj_add_event_cb(btn, event_handler_win2, LV_EVENT_CLICKED, NULL);
    lv_obj_t *cont = lv_win_get_content(win2); /*Content can be added here*/

    light_Arc1 = lv_arc_create(cont);
    lv_obj_set_width(light_Arc1, 150);
    lv_obj_set_height(light_Arc1, 143);
    lv_obj_set_x(light_Arc1, 0);
    lv_obj_set_y(light_Arc1, 0);
    lv_obj_set_align(light_Arc1, LV_ALIGN_CENTER);
    lv_arc_set_range(light_Arc1, 10, 100);
    lv_arc_set_value(light_Arc1, val);
    lv_obj_add_event_cb(light_Arc1, value_changed_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    light_Arc1_label = lv_label_create(light_Arc1);
    lv_obj_set_width(light_Arc1_label, LV_SIZE_CONTENT);  /// 1
    lv_obj_set_height(light_Arc1_label, LV_SIZE_CONTENT); /// 1
    lv_obj_set_x(light_Arc1_label, -4);
    lv_obj_set_y(light_Arc1_label, -7);
    lv_obj_set_align(light_Arc1_label, LV_ALIGN_CENTER);
    lv_label_set_text_fmt(light_Arc1_label, "%d%", val);
    lv_obj_set_style_text_font(light_Arc1_label, &lv_font_montserrat_30, LV_PART_MAIN | LV_STATE_DEFAULT);
}
...
void _lv_demo_music_resume(void)
{
    ...
    lv_anim_set_time(&a, ((spectrum_len - spectrum_i) * 1000));
    ...
    const char *filename = _lv_demo_music_get_title(track_id);
    Serial.println(String(filename));
    char chbuf[50];
    sprintf(chbuf, filename, "");
    audio.stopSong();
    delay(50);
...
static void create_wave_images(lv_obj_t *parent)
{
    ...
    lv_obj_align(wave_top, LV_ALIGN_TOP_MID, 0, 20);
    ...
    lv_obj_align(wave_bottom, LV_ALIGN_BOTTOM_MID, 0, -20);
...
static lv_obj_t *create_vol(lv_obj_t *parent)
{
    lv_obj_t *cont = lv_slider_create(parent);
    //    lv_obj_remove_style_all(cont);
    lv_obj_set_height(cont, 8);
    lv_obj_set_width(cont, 100);
    lv_obj_align(cont, LV_ALIGN_TOP_MID, 0, 30);

    //    lv_obj_set_flex_flow(cont, LV_FLEX_FLOW_ROW);
    //    lv_obj_set_flex_align(cont, LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

    lv_obj_add_event_cb(cont, slider_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    /*Create a label below the slider*/
    slider_label = lv_label_create(parent);
    lv_label_set_text(slider_label, "20");
    lv_slider_set_range(cont, 0, 20);
...
static void vol_click_event_cb(lv_event_t *e)
{
    lv_example_win_1();
}

static void light_click_event_cb(lv_event_t *e)
{
    lv_example_win_2();
}
...
//    lv_obj_set_style_pad_bottom(cont, 2, 0);
#endif
    static const lv_coord_t grid_col[] = {LV_GRID_FR(2), LV_GRID_FR(3), LV_GRID_FR(5), LV_GRID_FR(5), LV_GRID_FR(5),
    ...
    lv_obj_t *icon;
    icon = lv_btn_create(cont);
    lv_obj_set_width(icon, 45);
    lv_obj_set_height(icon, 45);
    lv_obj_add_flag(icon, LV_OBJ_FLAG_SCROLL_ON_FOCUS); /// Flags
    ...
    lv_obj_set_style_bg_img_src(icon, &img_lv_demo_music_btn_rnd, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(icon, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(icon, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_grid_cell(icon, LV_GRID_ALIGN_START, 1, 1, LV_GRID_ALIGN_CENTER, 0, 1);
    lv_obj_add_event_cb(icon, vol_click_event_cb, LV_EVENT_CLICKED, NULL);
    ...
    lv_obj_set_style_bg_color(icon, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(icon, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_img_src(icon, &img_lv_demo_music_btn_loop, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(icon, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(icon, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_grid_cell(icon, LV_GRID_ALIGN_START, 5, 1, LV_GRID_ALIGN_CENTER, 0, 1);
    lv_obj_add_event_cb(icon, light_click_event_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_set_style_shadow_width(icon, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    //    lv_img_set_src(play_obj, &img_lv_demo_music_btn_play);
    ...
    lv_obj_set_style_pad_all(slider_obj, 15, LV_PART_KNOB);
